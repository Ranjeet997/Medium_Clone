...
export default function SignUpNav(props) {
  ...
  return (
    <div>
      <nav className={`fixed top-0 w-full h-[6.5rem] md:h-[5.5rem] md:h-[5rem] font-satoshi text-black border-b border-black md:px-[2rem] lg:px-[2rem] 2xl:pl-2 2xl:pr-8 transition-all duration-300 ${isScrolled ? "bg-white" : "bg-[#F24D2E]"}`}>
        <div className="md:flex items-center md:justify-between lg:justify-between py-[.8rem] md:py-[1.15rem]">
          ...
          <div className="flex items-center justify-around md:justify-around border-t border-black md:border-t-0 mt-3.5 md:mt-0 lg:mt-0">
            ...
            <button
              className="text-white text-sm md:text-md lg:text-[1rem] lg:font-medium rounded-full absolute top-[1rem] right-5 md:relative md:top-0 lg:relative lg:top-0 lg:left-0 px-4 py-2 md:py-1.5 lg:py-[.5rem] 2xl:px-7 transition-all duration-200
            ...
            <div className="">
              <SignUp isOpen={isOpen} closeModel={closeModel} />
              </div>
          </div>
        </div>
      </nav>
