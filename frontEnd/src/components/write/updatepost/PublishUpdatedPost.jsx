...
export default function PublushUpdatedPost({
  visible,
  onClose,
  state1,
  postUid,
  ...
  const handleBlogPost = async (e) => {
    ...
    try {
      ...
      const post_uid = postUid;
      const post_tags = `${postTopics}`;
      const updated_at = "";

      const res = await fetch("http://localhost:8080/api/blog/updateblog", {
        method: "PUT",
      ...
      if (data.status === 409) {
        ...
        window.alert("seccessfuly updated post/blog");
